...
async function testAgentUpdateEmail() {
  try {
    ...
    const result = await emailService.sendAgentUpdateEmail({
      email: TEST_EMAIL,
      name: 'Test User',
      title: 'Test: New AI Agents Available',
      content: '<p>This is a test email to verify the agent update email functionality.</p>' +
               '<p>Check out our latest AI agents that can help you with various tasks!</p>',
      latestAgents: sampleAgents // Provide the sample agents directly
    });
